  ...
  let w = (v, T) => {
    ...
    P = (v, T, R) => {
      ...
      return (
        ...
        `__remixContext.r(${JSON.stringify(v)}, ${JSON.stringify(T)}, !1, ${ni(
          JSON.stringify(O)
        )})`
      );
    },
    p = (v, T, R) => {
      ...
      return `__remixContext.r(${JSON.stringify(v)}, ${JSON.stringify(T)}, ${ni(
        O
      )})`;
    },
    d = [],
              ...
              ee = X.deferredKeys.map((ke) => {
                if (ge.has(ke))
                  return (
                    ...
                    `${JSON.stringify(ke)}:__remixContext.n(${JSON.stringify(
                      V
                    )}, ${JSON.stringify(ke)})`
                  );
                {
...
`) +
      ...
      let M = s
        ? `${
            (v = t.hmr) !== null && v !== void 0 && v.runtime
...
window.__remixRouteModules = {${x
            .map((V, X) => `${JSON.stringify(V.route.id)}:route${X}`)
            .join(',')}};
